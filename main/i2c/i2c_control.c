...
/**
 * HP çekirdekteki ana I2C master (I2C_FIRST_MASTER_NUM) için
 * driver ve pin konfigürasyonunu yapar.
 */
...
/**
 * LP-core tarafında çalışan I2C master için konfigürasyon yapar.
 */
void init_lp_i2c_master(void)
{
    ...
    esp_err_t ret = lp_core_i2c_master_init(I2C_SECOND_MASTER_NUM,
...
/**
 * Verilen device/register adresine length byte veri yazar.
 * HP tarafında I2C_FIRST_MASTER_NUM veya I2C_SECOND_MASTER_NUM üzerinden çalışır.
 */
esp_err_t write_register(uint8_t device_address,
                         uint8_t reg_address,
                         uint8_t *data,
                         size_t length,
                         uint8_t i2c_master_number)
{
    ...
    ret = i2c_master_write_byte(cmd,
                                (uint8_t)((device_address << 1) | I2C_MASTER_WRITE),
                                true);
    if (ret != ESP_OK) goto cleanup;
    ...
    ret = i2c_master_cmd_begin(i2c_master_number,
                               cmd,
                               pdMS_TO_TICKS(10));
    if (ret != ESP_OK) {
        ESP_LOGE(TAG,
                 "I2C write failed: dev=0x%02X, reg=0x%02X, err=0x%x",
...
/**
 * Verilen device/register adresinden length byte veri okur.
 */
                        ...
                        uint8_t i2c_master_number)
{
    ...
    // Okuma için tekrar start
    ret = i2c_master_start(cmd);
    if (ret != ESP_OK) goto cleanup;

    ret = i2c_master_write_byte(cmd,
                                (uint8_t)((device_address << 1) | I2C_MASTER_READ),
                                true);
    if (ret != ESP_OK) goto cleanup;
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG,
                 "I2C read failed: dev=0x%02X, reg=0x%02X, err=0x%x",
                 device_address, reg_address, ret);
