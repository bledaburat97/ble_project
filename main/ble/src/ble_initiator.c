...
// BLE controller + Bluedroid stack başlatır.
...
// Security, callback ve advertising ayarlarını başlatır.
...
void register_on_connect_callback(void (*cb)(void)) {
    on_connect_callback = cb;
}

void register_on_disconnect_callback(void (*cb)(void)) { 
    on_disconnect_callback = cb;
}

void register_on_write_activation_callback(void (*cb)(const uint8_t *buf, size_t len)) { 
...
void register_on_write_updating_passkey_callback(void (*cb)(const uint8_t *buf, size_t len)) { 
    on_write_updating_passkey_callback = cb;
}
void register_on_write_updating_configuration_callback(void (*cb)(const uint8_t *buf, size_t len)) { 
    on_write_updating_configuration_callback = cb;
}
void register_on_write_wifi_config_callback(void (*cb)(const uint8_t *buf, size_t len)) {
    on_write_wifi_config_callback = cb;
}
void register_dynamic_period_change_callback(void (*cb)(uint16_t)) { 
    on_dynamic_period_change_callback = cb;
}

// BLE altyapısının ana giriş noktası.
void init_ble() {
