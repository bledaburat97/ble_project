...
// BLE TX gücünü maksimum seviyeye çeker.
void set_ble_tx_power(void) {
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to set ADV power: %s", esp_err_to_name(err));
    }
    err = esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_SCAN,  ESP_PWR_LVL_P20);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to set SCAN power: %s", esp_err_to_name(err));
    }
...
static esp_err_t set_phy_masks(const esp_bd_addr_t addr,
